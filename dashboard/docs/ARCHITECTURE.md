...
flowchart LR
    ...
    WH --> GEN["generate_training_data.py<br/>(real execute_plan calls,<br/>never an LLM guess)"]
    GEN --> J1["generated_questions.jsonl<br/>126 rows / 29 families"]
    GEN --> J2["generated_questions_large.jsonl<br/>939 rows / 32 families"]
```
...
### 6.1 The Eight Stages, Data to Served Model (file-in / file-out, each re-runnable alone)

```mermaid
flowchart LR
    J["generated_questions*.jsonl"] --> I[ingest] --> C[curate] --> R[render] --> T[train] --> P[predict] --> E[evaluate] --> S[select] --> X[export]
    X --> MA["merge-adapter.sh<br/>(fold LoRA into base, CPU)"]
    MA --> RV["run-vllm.sh<br/>(serve merged model, GPU)"]
    RV --> SUB[submission.json]
```
...
`src/data/generate_training_data.py` produces fine-tune data by **actually running TFQL `execute_plan` against the real warehouse** — never by asking an LLM to invent a number. Three grounded categories:
...
| file | rows | families | wired into a config? |
|---|---:|---:|---|
| `generated_questions.jsonl` | 126 | 29 | yes — `config/generated.yaml`'s field map is settled against it |
| `generated_questions_large.jsonl` | 939 | 32 | not yet — landed most recently, `adapter.path` just needs pointing at it |

**Current status:** the checkpoint actually merged & served today (`nemotron-8b-finance-merged` in `submission.json`) came from `config/nemotron.yaml` — a real LoRA run against 200 **stub** records, proving the GPU path end to end. `config/generated.yaml` (the real, grounded data above) has only been run through `ingest → curate → render → predict[base]` so far — `train`/`evaluate`/`select`/`export` on real data is the next step, not yet the source of the served model.
...
training/
  data/generated_questions*.jsonl # TFQL-grounded fine-tune data (126 rows; 939-row large variant not yet wired)
  ft-pipeline/                    # ★ staged 8-stage LoRA pipeline (ftpipe)

scripts/                          # merge-adapter.sh, run-vllm.sh: checkpoint -> served endpoint
